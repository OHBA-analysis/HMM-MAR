...
Sind = metastates.train.Sind;
regressed = sum((metastates.train.S==1),1)>0;
...
for k = 1:K
    if update==1 && isfield(metastate_noisy.state(1),'W') && ~isempty(metastate_noisy.state(1).W.Mu_W)
        ...
        if strcmp(metastate_noisy.train.covtype,'full') || strcmp(metastate_noisy.train.covtype,'uniquefull')
            ...
            for n = 1:size(metastates.state(k).W.S_W,1)
                metastates.state(k).W.S_W(n,Sind(:,n),Sind(:,n)) = ...
                    inv(permute(metastates.state(k).W.iS_W(n,Sind(:,n),Sind(:,n)),[2 3 1]));
            end
        end
    elseif update==2 && isfield(metastate_noisy.state(1),'Omega')
        ...
        if strcmp(metastate_noisy.train.covtype,'full') || strcmp(metastate_noisy.train.covtype,'uniquefull')
            metastates.state(k).Omega.Gam_irate(regressed,regressed) = ...
                inv(metastates.state(k).Omega.Gam_rate(regressed,regressed));
        else
            metastates.state(k).Omega.Gam_irate(regressed) = 1 ./ metastates.state(k).Omega.Gam_rate(regressed);
        end

...
responses = responses(:,:,1); % N x q
pval = [];  

Ystar = repmat(Y(1,:,:),[10,1,1]);
constant_trial = all(Y(:)==Ystar(:));
clear Ystar
max_num_classes = 5;
...
if CVmethod == 3
    try tuda = options.tuda; 
    catch, error('You need to supply options.tuda for CVmethod == 3')
    end
    if ~constant_trial
        error('To use CVmethod == 3, you need to have the same stimulus for the entire trial'); 
    end
end
...
stratified = length(unique(Y(:))) < max_num_classes;
...
% Intermediate estimation of Y, and re-estimation of Gamma
if CVmethod == 3
    Ypred = reshape(CV_estimation(X,Y,Gamma,Gammapred,c),[ttrial*N q]);
    Ypred = reshape(continuous_prediction_2class(Y,Ypred),[ttrial N q]);
    for jj = 1:N
        for j = 1:q
            Ypred(:,
        end
...
if mode == 1 %  time point by time point CV (i.e. not across time points)
    ...
    for r = 1:Nperm
        ...
        Ypred = CV_estimation(X,Y,G,Gp,c); % ttrial x N x q
...
% Standard CV estimation, time point by time point
function Ypred = CV_estimation(X,Y,G,Gp,c)
NCV = length(c);

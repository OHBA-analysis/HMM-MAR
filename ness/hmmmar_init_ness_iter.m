function Gamma = hmmmar_init_ness_iter(data,T,options)
%
...
while r <= options.initrep
    ...
    if any(sum(Gamma)==0)
        disp(['Rep : ' num2str(r) '. Unable to initialize ' num2str(K) ...
                ' chains, will keep ' num2str(Kfinal) ])
        %badinits = badinits + 1;
    ...
    c = 0;
    for k = 1:Kfinal % differences from baseline
        w_k = hmm.state(k).W.Mu_W(:);
        c = c + sum( (w_k - w_bas).^2 );
    end
    if c > cbest, Gammabest = Gamma(:,notzero); cbest = c; end    

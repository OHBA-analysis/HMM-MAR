function [errY,fracerrY,errR,fracerrR,response] = hmmerror(X,T,hmm,...
    Gamma,test,residuals,actstates)
%
...
% test      sum(T) x 1 vector indicating in which time points 
%               the error is to be computed;
%           common use will be to set test(t) = 1 when Gamma is NaN at this point
% residuals     in case we train on residuals, the value of those.
% actstates     Kx1 vector indicating which states were effectively 
...
if nargin<6 || isempty(residuals)
    residuals = ...
        getresiduals(X,T,train.S,train.maxorder,train.order,train.orderoffset,...
        train.timelag,train.exptimelag,train.zeromean);
end
if nargin<7
...
for j = 1:N
    t0 = sum(T(1:j-1));
    Y = [Y; X(t0+1+train.maxorder:t0+T(j),:)];
    te = [te; test(t0+1+train.maxorder:t0+T(j))];
end
...
fracerrY = errY ./ mean( (Y(te==1,regressed) - ...
    repmat(mean(Y(te==1,regressed)),sum(te),1) ).^2 );
errR = mean((residuals(te==1,regressed) - responseR(:,regressed)).^2);
fracerrR = errR ./ mean( (residuals(te==1,regressed) - ...
    repmat(mean(residuals(te==1,regressed)),sum(te),1) ).^2 );
